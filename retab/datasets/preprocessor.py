...
class Preprocessor:
    ...
    def _encode_onehot(self):
        ...
        print(f"One-hot Encoding for {self.categorical_columns}")
    ...
    def _compute_scaling_stats(self):
        ...
        sub_data = data[:, normalize_mask].astype(float)
    ...
    def _make_dataset(self, X, y):
        ...
        # Note that below data denotes ???????????????????????
        # I mean if OHE is used, then X, y should be already processed. 
        # However, X_train, y_train, X_test, y_test are not processed. ?????????????
